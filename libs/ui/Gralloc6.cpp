...
static const Gralloc6& getInstance() {
    static Gralloc6 instance = []() {
        auto allocator = waitForAllocator();
        if (!allocator) {
            return Gralloc6{};
        }
        ...
        return Gralloc6{std::move(allocator), mapper};
    }();
    return instance;
}
...
static auto getStandardMetadata(AIMapper* mapper, buffer_handle_t bufferHandle)
    ...
    int32_t sizeRequired = mapper->v6.getStandardMetadata(bufferHandle, static_cast<int64_t>(T),
    ...
    if ((size_t)sizeRequired > buffer.size()) {
        ...
        sizeRequired = mapper->v6.getStandardMetadata(bufferHandle, static_cast<int64_t>(T),
                                                      buffer.data(), buffer.size());
    }
...
static AIMapper_Error setStandardMetadata(AIMapper* mapper, buffer_handle_t bufferHandle,
    ...
    return mapper->v6.setStandardMetadata(bufferHandle, static_cast<int64_t>(T), buffer.data(),
                                          sizeRequired);
}

Gralloc6Allocator::Gralloc6Allocator(const Gralloc6Mapper& mapper) : mMapper(mapper) {
    mAllocator = getInstance().allocator;
}

bool Gralloc6Allocator::isLoaded() const {
...
std::string Gralloc6Allocator::dumpDebugInfo(bool less) const {
    return mMapper.dumpBuffers(less);
}

status_t Gralloc6Allocator::allocate(std::string requestorName, uint32_t width, uint32_t height,
...
GraphicBufferAllocator::AllocationResult Gralloc6Allocator::allocate(
...
Gralloc6Mapper::Gralloc6Mapper() {
    mMapper = getInstance().mapper;
}

bool Gralloc6Mapper::isLoaded() const {
    return mMapper != nullptr && mMapper->version >= AIMAPPER_VERSION_6;
}
...
std::string Gralloc6Mapper::dumpBuffer(buffer_handle_t bufferHandle, bool less) const {
    ...
    AIMapper_Error error = mmapper->v6.dumpBuffer(bufferHandle, dumpBuffer, &bufferInfo);
    if (error != AIMAPPER_ERROR_NONE) {
...
std::string Gralloc6Mapper::dumpBuffers(bool less) const {
    ...
    AIMapper_Error error = mmapper->v6.dumpAllBuffers(beginCallback, dumpBuffer, &context);
    if (error != AIMAPPER_ERROR_NONE) {
...
status_t Gralloc6Mapper::importBuffer(const native_handle_t* rawHandle,
                                      buffer_handle_t* outBufferHandle) const {
    return mmapper->v6.importBuffer(rawHandle, outBufferHandle);
}

void Gralloc6Mapper::freeBuffer(buffer_handle_t bufferHandle) const {
    mmapper->v6.freeBuffer(bufferHandle);
}

status_t Gralloc6Mapper::getMultiViewInfo(buffer_handle_t bufferHandle, BufferView* outViewList,
...
status_t Gralloc6Mapper::getBaseView(buffer_handle_t buffer, BufferView* outViewIndex) const {
    AIMapper_Error error =
            mMapper->v6.getBaseView(buffer, reinterpret_cast<uint32_t*>(outViewIndex));
    return error;
}

status_t Gralloc6Mapper::importViewBuffer(buffer_handle_t multiViewHandle, uint32_t viewIndex,
                                          buffer_handle_t* outBufferHandle) const {
    AIMapper_Error error =
...
status_t Gralloc6Mapper::validateBufferSize(buffer_handle_t bufferHandle, uint32_t width,
                                            uint32_t height, PixelFormat format,
                                            uint32_t layerCount, uint64_t usage,
...
void Gralloc6Mapper::getTransportSize(buffer_handle_t bufferHandle, uint32_t* outNumFds,
                                      uint32_t* outNumInts) const {
    mmapper->v6.getTransportSize(bufferHandle, outNumFds, outNumInts);
}

status_t Gralloc6Mapper::lock(buffer_handle_t bufferHandle, uint64_t usage, const Rect& bounds,
                              int acquireFence, void** outData, int32_t* outBytesPerPixel,
                              int32_t* outBytesPerStride) const {
    if (outBytesPerPixel) *outBytesPerPixel = -1;
    if (outBytesPerStride) *outBytesPerStride = -1;

    auto status = mmapper->v6.lock(bufferHandle, usage, bounds, acquireFence, outData);
...
int Gralloc6Mapper::unlock(buffer_handle_t bufferHandle) const {
    int fence = -1;
    AIMapper_Error error = mmapper->v6.unlock(bufferHandle, &fence);
    if (error != AIMAPPER_ERROR_NONE) {
...
status_t Gralloc6Mapper::isSupported(uint32_t width, uint32_t height, PixelFormat format,
                                     uint32_t layerCount, uint64_t usage,
...
status_t Gralloc6Mapper::getBufferId(buffer_handle_t bufferHandle, uint64_t* outBufferId) const {
...
status_t Gralloc6Mapper::getName(buffer_handle_t bufferHandle, std::string* outName) const {
...
status_t Gralloc6Mapper::getWidth(buffer_handle_t bufferHandle, uint64_t* outWidth) const {
    auto value = getStandardMetadata<StandardMetadataType::WIDTH>(mMapper, bufferHandle);
    if (value.has_value()) {
        *outWidth = *value;
        return OK;
...
status_t Gralloc6Mapper::getHeight(buffer_handle_t bufferHandle, uint64_t* outHeight) const {
    auto value = getStandardMetadata<StandardMetadataType::HEIGHT>(mMapper, bufferHandle);
    if (value.has_value()) {
        *outHeight = *value;
        return OK;
...
status_t Gralloc6Mapper::getLayerCount(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getPixelFormatRequested(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getPixelFormatFourCC(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getPixelFormatModifier(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getUsage(buffer_handle_t bufferHandle, uint64_t* outUsage) const {
    auto value = getStandardMetadata<StandardMetadataType::USAGE>(mMapper, bufferHandle);
    if (value.has_value()) {
...
status_t Gralloc6Mapper::getAllocationSize(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getProtectedContent(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getCompression(
...
status_t Gralloc6Mapper::getCompression(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getInterlaced(
        buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getInterlaced(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getChromaSiting(
        buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getChromaSiting(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getPlaneLayouts(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getDataspace(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::setDataspace(buffer_handle_t bufferHandle, ui::Dataspace dataspace) const {
    return setStandardMetadata<StandardMetadataType::DATASPACE>(mMapper, bufferHandle,
                                                                static_cast<Dataspace>(dataspace));
}

status_t Gralloc6Mapper::getBlendMode(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getSmpte2086(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::setSmpte2086(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getCta861_3(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::setCta861_3(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getSmpte2094_40(
...
status_t Gralloc6Mapper::setSmpte2094_40(buffer_handle_t bufferHandle,
...
status_t Gralloc6Mapper::getSmpte2094_10(
...
status_t Gralloc6Mapper::setSmpte2094_10(buffer_handle_t bufferHandle,
