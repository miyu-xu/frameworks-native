...
/**
 * Advertise the port number the TLS server runs on.
 *
 * @param ctx the AdbdAuthContext
 * @param port the port number the TLS server is running on. 0 = not running.
 */
void adbd_auth_send_tls_server_port(AdbdAuthContext* ctx, uint16_t port) __INTRODUCED_IN(36);
