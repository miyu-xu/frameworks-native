...
static_assert(
        ...
        "HAL and AHardwareBuffer pixel format don't match");
static_assert(static_cast<int>(aidl::android::hardware::graphics::common::PixelFormat::YCBCR_P210) == AHARDWAREBUFFER_FORMAT_YCbCr_P210,
              "HAL and AHardwareBuffer pixel format don't match");
...
int AHardwareBuffer_lockPlanes(AHardwareBuffer* buffer, uint64_t usage,
    ...
    if (AHardwareBuffer_formatIsYuv(format)) {
      ...
      if (result == 0) {
        ...
        // P010 & P210 is word-aligned 10-bit semiplaner, and YCbCr_422_I is a single interleaved plane
