...
bool ClientCounterCallbackImpl::registerServiceLocked(const sp<IBinder>& service,
    ...
    if (dumpFlags & android::os::IServiceManager::DUMP_FLAG_IS_LAZY_SERVICE) {
        ALOGW("DUMP_FLAG_IS_LAZY_SERVICE flag already set. This should only be set by "
              "ClientCounterCallbackImpl in LazyServiceRegistrar");
    }
    dumpFlags |= android::os::IServiceManager::DUMP_FLAG_IS_LAZY_SERVICE;
