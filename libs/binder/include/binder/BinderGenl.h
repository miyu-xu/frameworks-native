...
// Copy of binder genl header in case include/uapi/linux/*.h is not new enough
...
/**
 * enum binder_genl_flag - Define what kind of binder transactions should be
 *   reported.
 */
...
public:
    ...
    LIBBINDER_EXPORTED int send(__u16 nlmsgType, __u8 cmd, __u16 nlaType,
                                const void *nlaData, __u16 nlaLen);
     ...
     */
    LIBBINDER_EXPORTED int send(__u16 nlmsgType, __u8 cmd, __u16 nlaType[],
                                const void *nlaData[], __u16 nlaLen[], int n);
     ...
     * @param flags   The flags to be set (see enum binder_genl_flag)
     * @return 0 on success, or -1 on error
     */
    LIBBINDER_EXPORTED int setReport(const char *context, __u32 pid, __u32 flags);

    /**
     * Receive next binder genl report.
     *
     * @param context The name of the binder context
     * @param size    The buffer size of the parameter context
     * @param report  The u32 array to store the binder genl report
     * @return 0 on success, or -1 on error
     */
    LIBBINDER_EXPORTED int getReport(char *context, int size,
