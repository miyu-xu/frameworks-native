...
status_t IPCThreadState::getAndExecuteCommand()
{
    ...
    if (result >= NO_ERROR) {
        ...
        if (newThreadsCount >= mProcess->mMaxThreads) {
            auto expected = mProcess->mStarvationStartTime.load().min();
            auto desired = std::chrono::steady_clock::now();
            mProcess->mStarvationStartTime.compare_exchange_strong(expected, desired);
        }
        ...
        if (newThreadsCount < maxThreads) {
            auto starvationStartTime = mProcess->mStarvationStartTime.exchange(
                    mProcess->mStarvationStartTime.load().min());
            if (starvationStartTime != mProcess->mStarvationStartTime.load().min()) {
