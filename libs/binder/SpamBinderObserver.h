...
namespace android {
class SpamBinderObserver : public IBinderObserver {
public:
    SpamBinderObserver() {
        mStorage.reserve(20);
...
private:
    ...
    typedef std::map<uint32_t /*workSourceUid*/, DataPerWorkSourceUid> SpamDataPerTxnCode;
    void flushDataForCurrentEpochSecond(std::chrono::seconds epochSecondBucket);
    std::vector<std::pair<uint32_t /*transactionCode*/, SpamDataPerTxnCode>> mStorage;
