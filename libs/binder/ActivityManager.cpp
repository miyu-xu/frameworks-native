...
status_t ActivityManager::getExecutableMethodFileOffsets(
        const app::TargetProcess& targetProcess, const app::MethodDescriptor& methodDescriptor,
        app::ExecutableMethodFileOffsets* out) {
    sp<IActivityManager> service = getService();
    if (service != nullptr) {
        return service->getExecutableMethodFileOffsets(targetProcess, methodDescriptor, out);
    }
    return INVALID_OPERATION;
}
