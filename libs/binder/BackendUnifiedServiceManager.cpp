...
binder::Status BackendUnifiedServiceManager::getService(const ::std::string& name,
    ...
    binder::ScopedTrace trace(ATRACE_TAG_AIDL, ("BackendUnifiedServiceManager getService " + name).c_str());
    if (name == "carrier_config") {
        ALOGE("BackendUnifiedServiceManager getService carrier_config");
    }
    os::Service service;
...
binder::Status BackendUnifiedServiceManager::getService2(const ::std::string& name,
                                                         os::Service* _out) {
    binder::ScopedTrace trace(ATRACE_TAG_AIDL, ("BackendUnifiedServiceManager getService2 " + name).c_str());
    if (name == "carrier_config") {
        ALOGE("BackendUnifiedServiceManager getService2 carrier_config");
    }
...
binder::Status BackendUnifiedServiceManager::checkService(const ::std::string& name,
                                                          os::Service* _out) {
    binder::ScopedTrace trace(ATRACE_TAG_AIDL, ("BackendUnifiedServiceManager checkService " + name).c_str());
    if (name == "carrier_config") {
