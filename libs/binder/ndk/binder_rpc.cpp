...
ARpc_AccessorProvider* ARpc_addAccessorProvider(ARpc_AccessorProvider_getAccessor provider,
    ...
    std::function<sp<IBinder>(const String16& name)> generate =
            [provider, onDeleteHolder](const String16& name) -> sp<IBinder> {
...
binder_status_t ARpc_removeAccessorProvider(ARpc_AccessorProvider* provider) {
    ...
    android::status_t status = android::removeAccessorProvider(provider->mProviderCookie);
    delete provider;
...
ARpc_Accessor* ARpc_Accessor_new(const char* instance, ARpc_ConnectionInfoProvider provider,
    ...
    if (provider) {
        generate = [provider, onDeleteHolder](const String16& name, sockaddr* outAddr,
                                              size_t addrLen) {
            ARpc_ConnectionInfo* info = provider(String8(name).c_str(), onDeleteHolder->mData);
            if (info == nullptr) {
