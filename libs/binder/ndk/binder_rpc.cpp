...
ABinderRpc_AccessorProvider* ABinderRpc_registerAccessorProvider(
        ...
        uint32_t numInstances, void* data,
    ...
    if (numInstances == 0 || instances == nullptr) {
        ALOGE("No instances passed to ABinderRpc_registerAccessorProvider. numInstances: %d",
              numInstances);
        return nullptr;
    }
...
void ABinderRpc_unregisterAccessorProvider(ABinderRpc_AccessorProvider* provider) {
    ...
    LOG_ALWAYS_FATAL_IF(status == android::BAD_VALUE, "Provider is not valid");
    LOG_ALWAYS_FATAL_IF(status == android::NAME_NOT_FOUND, "Provider was already unregistered");
    LOG_ALWAYS_FATAL_IF(
            status != OK,
            "Unknown error when attempting to unregister ABinderRpc_AccessorProvider. Status: %s",
            android::statusToString(status).c_str());
...
binder_status_t ABinderRpc_Accessor_delegateAccessor(const char* instance, AIBinder* accessor,
    ...
    if (instance == nullptr || accessor == nullptr) {
        ALOGE("Arguments to ABinderRpc_Accessor_delegateBinder are null");
