    ...
    while (cur < payload) {
        ...
        switch (nla->nla_type) {
            case CTRL_ATTR_FAMILY_NAME: {
                char* name = reinterpret_cast<char*>(NLA_DATA(nla));
                ALOGD("Binder Netlink family name is %s", name);
                break;
            }
            ...
            case CTRL_ATTR_MCAST_GROUPS: {
                ALOGD("Binder Netlink MCAST_GROUP");
                int cur2 = 0;
                int payload2 = nla->nla_len;
                char* data2 = reinterpret_cast<char*>(NLA_DATA(nla)) + NLA_HDRLEN;
                while (cur2 < payload2) {
                    ALOGV("Checking nested NLA payload %d / %d",
                          cur2,
                          payload2);
                    struct nlattr
                        * nla2 = reinterpret_cast<struct nlattr*>(data2 + cur2);
                    ALOGV("Nested NLA type / len: %d / %d",
                          nla2->nla_type,
                          nla2->nla_len);
                    if (nla2->nla_len == 0) break;
                    cur2 += NLA_ALIGN(nla2->nla_len);
                    switch (nla2->nla_type) {
                        case CTRL_ATTR_MCAST_GRP_NAME: {
                            char* name =
                                reinterpret_cast<char*>(NLA_DATA(nla2));
                            ALOGD("Binder Netlink mcast group name is %s",
                                  name);
                            break;
                        }
                        case CTRL_ATTR_MCAST_GRP_ID: {
                            __u32 grp =
                                *(reinterpret_cast<__u32*>(NLA_DATA(nla2)));
                            ALOGD("Binder Netlink mcast group id is %d", grp);
                            setsockopt(mFd.get(), SOL_NETLINK, NETLINK_ADD_MEMBERSHIP, &grp, sizeof(grp));
                            break;
                        }
                        default:break;
                    }
