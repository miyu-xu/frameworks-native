...
enum BinderLibTestTranscationCode {
    ...
    BINDER_LIB_TEST_LISTEN_FOR_FREEZE_STATE_CHANGE,
...
class TestFreezeStateChangeCallback : public IBinder::FreezeStateChangeCallback {
public:
    BlockingQueue<std::pair<const wp<IBinder>, bool>> events;

    virtual void onStateChange(const wp<IBinder>& who, bool isFrozen) {
...
TEST_F(BinderLibTest, ReturnErrorIfKernelDoesNotSupportFreezeNotification) {
    ...
    sp<TestFreezeStateChangeCallback> callback = new TestFreezeStateChangeCallback();
    sp<IBinder> binder = addServer();
    ASSERT_TRUE(binder != nullptr);
    EXPECT_THAT(binder->addFreezeStateChangeCallback(callback), StatusEq(INVALID_OPERATION));
}

TEST_F(BinderLibTest, FreezeStateChangeNotificatiion) {
    ...
    EXPECT_THAT(binder->addFreezeStateChangeCallback(callback), StatusEq(NO_ERROR));
    ...
    unfreezeProcess(pid);
    callback->ensureUnfrozenEventReceived();
    EXPECT_THAT(binder->removeFreezeStateChangeCallback(callback), StatusEq(NO_ERROR));
    IPCThreadState::self()->flushCommands();
...
TEST_F(BinderLibTest, MultipleFreezeStateChangeCallbacks) {
    if (!checkFreezeAndNotificationSupport()) {
        GTEST_SKIP();
        return;
    }
    sp<TestFreezeStateChangeCallback> callback1 = new TestFreezeStateChangeCallback();
    sp<TestFreezeStateChangeCallback> callback2 = new TestFreezeStateChangeCallback();
    sp<IBinder> binder = addServer();
    ASSERT_TRUE(binder != nullptr);
    int32_t pid;
    getBinderPid(&pid, binder);

    EXPECT_THAT(binder->addFreezeStateChangeCallback(callback1), StatusEq(NO_ERROR));
    // Expect current state (unfrozen) to be delivered immediately.
    callback1->ensureUnfrozenEventReceived();

    EXPECT_THAT(binder->addFreezeStateChangeCallback(callback2), StatusEq(NO_ERROR));
    // Expect current state (unfrozen) to be delivered immediately.
    ...
    EXPECT_THAT(binder->removeFreezeStateChangeCallback(callback1), StatusEq(NO_ERROR));
    IPCThreadState::self()->flushCommands();
    unfreezeProcess(pid);
    EXPECT_EQ(callback1->events.size(), 0);
    callback2->ensureUnfrozenEventReceived();
    EXPECT_THAT(binder->removeFreezeStateChangeCallback(callback2), StatusEq(NO_ERROR));
    IPCThreadState::self()->flushCommands();
...
TEST_F(BinderLibTest, RemoveThenAddFreezeStateChangeCallbacks) {
    if (!checkFreezeAndNotificationSupport()) {
        GTEST_SKIP();
        return;
    }
...
TEST_F(BinderLibTest, CoalesceFreezeCallbacksWhenListenerIsFrozen) {
    ...
    // Ask the listener process to register for state change callbacks.
    {
        Parcel data, reply;
        data.writeStrongBinder(binder);
        ASSERT_THAT(listener->transact(BINDER_LIB_TEST_LISTEN_FOR_FREEZE_STATE_CHANGE, data,
                                       &reply),
                    StatusEq(NO_ERROR));
    }
    ...
    // Unfreeze the listener process. Now it should receive the freeze state
...
public:
    ...
    virtual status_t onTransact(uint32_t code, const Parcel &data, Parcel *reply,
        ...
        switch (code) {
            ...
            case BINDER_LIB_TEST_LISTEN_FOR_FREEZE_STATE_CHANGE: {
                sp<IBinder> binder = data.readStrongBinder();
                freezeStateChangeCallback = new TestFreezeStateChangeCallback();
                int ret = binder->addFreezeStateChangeCallback(freezeStateChangeCallback);
                if (ret != NO_ERROR) {
                    return ret;
                }
                auto event = freezeStateChangeCallback->events.popWithTimeout(10ms);
                if (!event.has_value()) {
            ...
            case BINDER_LIB_TEST_CONSUME_STATE_CHANGE_EVENTS: {
                reply->writeBoolVector(freezeStateChangeCallback->getAllAndClear());
                return NO_ERROR;
            }
...
private:
    ...
    sp<TestFreezeStateChangeCallback> freezeStateChangeCallback;
};
