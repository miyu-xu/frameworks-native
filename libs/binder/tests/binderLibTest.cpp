...
/**
 * Running BinderNetlink tests requires "adb shell stop". Otherwise the tests would fail as the
 * BinderNetlink socket is already used by AMS.
 */
    ...
    void monitorBinderReport() {
        ...
        while (!finished) {
            ...
            if (getReport(&report) < 0) {
                ...
                ALOGD("Binder report: %s %u 0x%08x %u:%u -> %u:%u %u 0x08%x %u %u", report.context,
                      report.data[BINDER_A_REPORT_ERR], report.data[BINDER_A_REPORT_ERR],
                      report.data[BINDER_A_REPORT_FROM_PID], report.data[BINDER_A_REPORT_FROM_TID],
                      report.data[BINDER_A_REPORT_TO_PID], report.data[BINDER_A_REPORT_TO_TID],
                      report.data[BINDER_A_REPORT_REPLY], report.data[BINDER_A_REPORT_FLAGS],
                      report.data[BINDER_A_REPORT_CODE], report.data[BINDER_A_REPORT_DATA_SIZE]);
                if (report.data[BINDER_A_REPORT_FROM_PID] == from &&
                    report.data[BINDER_A_REPORT_TO_PID] == to &&
                    report.data[BINDER_A_REPORT_CODE] == code &&
                    report.data[BINDER_A_REPORT_ERR] == err) {
            ...
            // Don't starve others in case there are huge amount of abnormal reports
            sched_yield();
        }
    }
...
public:
    ...
    std::thread monitorStart() {
        struct SetupAttr setup = {
                .context = "binder",
                .pid = 0,
                .flags = BINDER_FLAG_FAILED | BINDER_FLAG_ASYNC_FROZEN | BINDER_FLAG_SPAM,
        };
        EXPECT_EQ(NO_ERROR, setReport(&setup));
        return std::thread(&TestBinderNetlink::monitorBinderReport, this);
    }
