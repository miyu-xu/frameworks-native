...
public:
    ...
    binder::Status addService(const std::string& name, const sp<::IBinder>& service, bool allowIsolated, int32_t dumpPriority) override {
        return binder::Status::fromStatusT(innerSm.addService(String16(name.c_str()), service, allowIsolated, dumpPriority));
    }
...
TEST(LibbinderCache, AddServiceAndConfirmCacheHit) {
    sp<BackendUnifiedServiceManager> busm = sp<BackendUnifiedServiceManager>::make(
        sp<MockAidlServiceManager>::make());
