...
TEST(LibbinderCache, AddServiceAndGet) {
    ...
    String16 service_name = String16("permissionmgr");
    sp<IBinder> nullBinder = nullptr;

    // Add a service (in this case the default service manager)
    fakeservicemanager.addService(service_name, smBinder);
    // Get the service. This caches it.
    sp<IBinder> targetBinder = fakeservicemanager.getService(service_name);
    // Add the null binder and replace the service.
    // The cache should still hold the default service manager.
    fakeservicemanager.addService(service_name, nullBinder);

    EXPECT_EQ(targetBinder, fakeservicemanager.getService(service_name));
}
