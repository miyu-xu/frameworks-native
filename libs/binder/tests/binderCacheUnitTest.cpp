...
protected:
    void SetUp() override {
        ...
        mServiceManager = getServiceManagerShimFromAidlServiceManager(sm);
    }
...
public:
    ...
    void addBinderToSm(const std::string& kCachedServiceName, const sp<IBinder>& binder) {
        EXPECT_EQ(OK, mServiceManager->addService(String16(kCachedServiceName.c_str()), binder, false, 0));
    }
    void checkService(const std::string& serviceName, sp<IBinder>* _out) {
        *_out = mServiceManager->checkService(String16(serviceName.c_str()));
    }
