...
TEST_P(BinderRpc, SendTooLargeVector) {
    ...
    // TODO(b/392717039): Telling a server to allocate too much data currently causes it to exit
    // since RpcServer treats any transaction error as a failure. We likely want to change this
    // behavior to be a soft failure, since it isn't hard to keep track of this state.
