...
impl Accessor {
    ...
    /// that has the permissions to create the connection and generate the socket
    /// FD.
    ///
    /// The `instance` argument must match the instance that the original Accessor
    /// is responsible for.
    /// `instance` must be a valid C string and will passed through FFI.
    /// The `binder` arguement must be a valid binder from an Accessor
    pub fn from_binder(instance: &str, binder: SpIBinder) -> Option<Accessor> {
        ...
        // to an `AIBinder` that is guarenteed to remain valid for the lifetime of the
