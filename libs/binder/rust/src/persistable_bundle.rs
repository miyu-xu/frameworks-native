    ...
    unsafe fn get_vec<T: Clone>(
        ...
        match unsafe { get_func(self.0.as_ptr(), key.as_ptr(), null_mut(), 0) } {
            ...
            required_buffer_size => {
                ...
                // guaranteed to be valid for the lifetime of `key`. The value buffer pointer is
            ...
            Ok(Some(
                value
                    ...
                    .map(|s| {
                        ...
                        let raw_slice = slice_from_raw_parts_mut(s, string_length + 1);
                        // SAFETY: The pointer was returned from `string_allocator`, which used
                        ...
                        let c_string = CString::from_vec_with_nul(boxed_slice.into()).expect(
                            "APersistableBundle_getString returned string missing NUL byte",
                        );
                        c_string
                            .into_string()
