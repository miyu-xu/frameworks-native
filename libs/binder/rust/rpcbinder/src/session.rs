...
impl RpcSessionRef {
    ...
    pub fn setup_preconnected_client<T: FromIBinder + ?Sized>(
        ...
        let request_fd_box = Box::new(request_fd);
...
type RequestFdBox = Box<dyn FnMut() -> Option<RawFd>>;

unsafe extern "C" fn request_fd_wrapper(param: *mut c_void) -> c_int {
    let request_fd_ptr = param as *mut RequestFdBox;
...
unsafe extern "C" fn param_delete_fd_wrapper(param: *mut c_void) {
    ...
    let request_fd_box = unsafe { Box::from_raw(param as *mut RequestFdBox) };
    drop(request_fd_box);
}
