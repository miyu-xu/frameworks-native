...
int BinderGenl::send(__u16 nlmsgType, __u8 cmd, __u16 nlaType, void *nlaData,
    ...
    msg.glh.cmd = cmd;
    msg.glh.version = BINDER_GENL_VERSION;
...
void *BinderGenl::recv(struct GenlMsg *msg, int len, __u8 cmd, __u16 nlaType,
                       GenlCallback callback) {
    if (recv(msg, len) < 0)
        return NULL;
    ...
    ALOGV("Checking glh cmd: %d %d", glh->cmd, cmd);
    if (glh->cmd != cmd)
        return NULL;
    ...
    while (cur < payload) {
        ...
        if (nla->nla_type == nlaType) {
            ptr = NLA_DATA(nla);
            if (callback) {
                callback(ptr, nla->nla_len);
            } else {
                return ptr;
            }
        }
...
int BinderGenl::getFamilyId() {
    ...
    struct GenlMsg msg;
    void *p = recv(&msg, sizeof(msg), CTRL_CMD_NEWFAMILY, CTRL_ATTR_FAMILY_ID);
    if (!p) {
        ALOGE("Failed to find CTRL_CMD_NEWFAMILY/CTRL_ATTR_FAMILY_ID");
        return -1;
    }
...
int BinderGenl::setReport(__u32 pid, __u32 flags) {
    ...
    ALOGV("Sending BINDER_GENL_CMD_SET_REPORT %u %u", pid, flags);
    ret = send2(id, BINDER_GENL_CMD_SET_REPORT, BINDER_GENL_ATTR_PID, &pid, sizeof(pid),
                BINDER_GENL_ATTR_FLAGS, &flags, sizeof(flags));
    if (ret < 0) {
        ALOGE("Failed to send BINDER_GENL_CMD_SET_REPORT");
        return -1;
    }

    struct GenlMsg msg;
    if (recv(&msg, sizeof(msg)) < 0) {
        ALOGE("Failed to recv reply of BINDER_GENL_CMD_SET_REPORT");
        return -1;
    }
    ...
    if (nla->nla_type != BINDER_GENL_ATTR_PID) {
        ALOGE("Wrong nla type %d, expect %d", nla->nla_type, BINDER_GENL_ATTR_PID);
        return -1;
    }
    ...
    nla = NLA_NEXT(nla);
    if (nla->nla_type != BINDER_GENL_ATTR_FLAGS) {
        ALOGE("Wrong nla type %d, expect %d", nla->nla_type, BINDER_GENL_ATTR_FLAGS);
        return -1;
    }
    p = (__u32 *)NLA_DATA(nla);
    if (*p != flags) {
