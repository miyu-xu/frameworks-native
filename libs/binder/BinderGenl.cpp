...
int BinderGenl::setReport(__u32 pid, __u32 flags) {
    ...
    ret = send2(id, BINDER_GENL_CMD_SET, BINDER_GENL_A_ATTR_PID, &pid, sizeof(pid),
                BINDER_GENL_A_ATTR_FLAGS, &flags, sizeof(flags));
    if (ret < 0) {
    ...
    if (nla->nla_type != BINDER_GENL_A_ATTR_PID) {
        ALOGE("Wrong nla type %d, expect %d", nla->nla_type, BINDER_GENL_A_ATTR_PID);
        return -1;
    }
    ...
    nla = NLA_NEXT(nla);
    if (nla->nla_type != BINDER_GENL_A_ATTR_FLAGS) {
        ALOGE("Wrong nla type %d, expect %d", nla->nla_type, BINDER_GENL_A_ATTR_FLAGS);
        return -1;
    }
    p = (__u32 *)NLA_DATA(nla);
    if (*p != flags) {
