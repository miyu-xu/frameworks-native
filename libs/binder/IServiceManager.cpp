...
class BinderCacheWithInvalidation {
    class BinderInvalidation : public IBinder::DeathRecipient {
      public:
        BinderInvalidation(String16ToIBinderCache* cache, String16 key)
            : mInternalCache(cache), mKey(key) {}
        void binderDied(const android::wp<android::IBinder>& /* who */) override {
            mInternalCache->removeItem(mKey);
        }

      private:
        String16ToIBinderCache* mInternalCache;
        String16 mKey;
    };
public:
    std::optional<sp<IBinder>> getItem(const String16& key) {
        return mInternalCache.getItem(key);
    }
    void addItem(const String16 key, sp<IBinder> item) {
        mInternalCache.addItem(key, item);
        sp<BinderInvalidation> deathRecipient = sp<BinderInvalidation>::make(&mInternalCache, key);
        item->linkToDeath(deathRecipient);
    }
private:
    String16ToIBinderCache mInternalCache;
};
...
#ifdef LIBBINDER_CLIENT_CACHE
    if (ClientSideCacheList::allow_client_side_caching(name) && ret->isBinderAlive()) {
