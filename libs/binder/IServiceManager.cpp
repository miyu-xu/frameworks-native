...
[[clang::no_destroy]] static std::vector<std::shared_ptr<AccessorProvider>> gAccessorProviders;
...
public:
    ...
    android::binder::Status getService2(const std::string& name,
        ...
        std::vector<std::shared_ptr<AccessorProvider>> copiedProviders;
        {
        ...
        for (const auto& provider : copiedProviders) {
            sp<IBinder> binder = provider->provider(String16(name.c_str()));
...
status_t addAccessorProvider(std::function<sp<IBinder>(const String16& name)>&& provider,
                             std::function<void()>&& onDelete) {
    std::lock_guard<std::mutex> lock(gAccessorProvidersMutex);
    gAccessorProviders.push_back(
            std::make_shared<AccessorProvider>(std::move(provider), std::move(onDelete)));
