...
static std::unordered_set<std::u16string> static_cache_list = {
...
class BinderCacheWithInvalidation {
    ...
    public:
        BinderInvalidation(std::unordered_map<std::u16string, sp<IBinder>>& cache, std::mutex& lock,
    ...
    private:
        std::unordered_map<std::u16string, sp<IBinder>>& mCache;
        std::mutex& mLock;
        std::u16string mKey;
    };

public:
    sp<IBinder> getItem(const std::u16string& key) const {
    ...
    void addItem(const std::u16string& key, const sp<IBinder>& item) {
        std::u16string key_u16str = std::u16string(key);
        {
            std::unique_lock<std::mutex> lock(mCacheMutex);
    ...
    bool isClientSideCachingEnabled(const std::u16string& service_name) {
    ...
    void addToDynamicAllowList(const std::u16string& service_name) {
        mDynamicAllowCacheList.insert(service_name);
    }

private:
    static bool isClientSideCachingEnabledStatic(const std::u16string& service_name) {
        return static_cache_list.contains(service_name);
    }

    bool isClientSideCachingEnabledDynamic(const std::u16string& service_name) {
        return mDynamicAllowCacheList.contains(service_name);
    }

    std::unordered_map<std::u16string, sp<IBinder>> mCache;
    mutable std::mutex mCacheMutex;
    std::unordered_set<std::u16string> mDynamicAllowCacheList;
};
...
protected:
    ...
    mutable BinderCacheWithInvalidation mCacheForGetService;
...
{
    std::u16string name_u16str;
    if (kUseCache) {
        name_u16str = std::u16string(name);
        sp<IBinder> item = mCacheForGetService.getItem(name_u16str);
    ...
    if (kUseCache) {
        sp<IBinder> binder = ret.get<Service::Tag::binder>();
        if (binder && mCacheForGetService.isClientSideCachingEnabled(name_u16str) && binder->isBinderAlive()) {
            mCacheForGetService.addItem(name_u16str, binder);
        }
    }
