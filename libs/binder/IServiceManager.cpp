...
#include <android/os/IAccessor.h>
        >>>>>>> b05e315042(libbinder chnages to support VM to host communicaiton)
...
=======
        ...
        switch (service.getTag()) {
            ...
            case Service::Tag::accessor:
                ...
                ALOGE("Cast an accessor for %s", name.c_str());
                auto request = [=] {
                    os::ParcelFileDescriptor fd;
                    // I think this is going to keep the sp<IAccessor> around..
                ...
                // Why does this fail in
                // TODO getRootObject()->Excluesiveconnection::find()?
                session->setMaxOutgoingConnections(4);
                ALOGE("Calling setupPreconnectedClient");
                base::unique_fd initial_fd = request();
                if (initial_fd == -1) {
                    ALOGE("Can't get usable FD! it's -1");
                } else if (!initial_fd.ok()) {
                    ALOGE("Can't get usable FD! it's not .ok()");
                } else {
                    ALOGE("Got a usable FD for setupPreconnectedClient!");
                }
