...
bool CppBackendShim::checkServiceAccess(const String16& caller, pid_t callerPid, uid_t callerUid,
                                        const String16& service, const String16& permission) {
    bool res = false;
    os::IServiceManager::CallerContext callerCtx;
    callerCtx.sidName = String8(caller).c_str();
    callerCtx.debugPid = callerPid;
    callerCtx.uid = callerUid;

    if (Status status =
                mUnifiedServiceManager->checkServiceAccess(callerCtx, String8(service).c_str(),
                                                           String8(permission).c_str(), &res);
        !status.isOk()) {
        ALOGW("%s Failed to check callers access to service %s for permission %s", __FUNCTION__,
              String8(service).c_str(), String8(permission).c_str());
        return false;
    }
    return res;
}
