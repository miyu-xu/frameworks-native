...
using android::os::RemoteService;
        ...
        switch (service.getTag()) {
            case Service::Tag::binder:
                *_aidl_return = service.get<Service::Tag::binder>();
                break;
            case Service::Tag::remoteService:
                RemoteService& remoteService = service.get<Service::Tag::remoteService>();
                auto session = RpcSession::make();
                session->setupPreconnectedClient(remoteService.mFdToRpcService.release(),
                                                 []() -> base::unique_fd {
                                                     ALOGE("Invalid Fd in remote service");
                                                     return base::unique_fd(0);
                                                 });
                session->getRootObject()->localBinder()->setExtension(remoteService.mAccessor);
