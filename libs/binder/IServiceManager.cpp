        ...
        if (pc != nullptr) {
            ...
            if (res) {
                if (startTime != startTime.min()) {
                    ...
                    ALOGI("Check passed after %sms for %s from uid=%d pid=%d",
                          to_string_ms(waitTime).c_str(), String8(permission).c_str(), uid, pid);
                }
                return res;
            }
...
sp<IBinder> ServiceManagerShim::getService(const String16& name) const
{
    ...
    while (std::chrono::steady_clock::now() - startTime < timeout) {
        ...
        if (svc != nullptr) {
            const auto waitTime = std::chrono::steady_clock::now() - startTime;
            ALOGI("Waiting for service '%s' on '%s' successful after waiting %sms",
                  String8(name).c_str(), ProcessState::self()->getDriverName().c_str(),
                  to_string_ms(waitTime).c_str());
            return svc;
        }
    }
