...
public:
    ...
    ~LocalAccessor() {
        ALOGI("LocalAccessor for %s being destructed", String8(mInstance).c_str());
        if (mOnDelete) mOnDelete();
    }

    ::android::binder::Status addConnection(::android::os::ParcelFileDescriptor* outFd) {
        using android::os::IAccessor;
        ALOGI("Setting up the client socket connection for %s", String8(mInstance).c_str());
        ...
        if (addrStorage.ss_family == AF_VSOCK) {
            ...
            status = singleSocketConnection(UnixSocketAddress(addr->sun_path), trigger.get(), &fd);
        ...
        if (status == OK) {
            ALOGI("Successfully created the client socket connection for %s.",
                  String8(mInstance).c_str());
    ...
    ::android::binder::Status getInstanceName(String16* instance) {
        LOG_ALWAYS_FATAL_IF(instance == nullptr, "instance must never be null");
        *instance = mInstance;
        return Status::ok();
    }
