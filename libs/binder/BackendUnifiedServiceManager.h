    ...
    public:
        ...
        void binderDied(const wp<IBinder>& who) override {
            ...
            if (std::shared_ptr<BinderCacheWithInvalidation> cache = mCache.lock()) {
                ...
                ALOGW("Binder Cache pointer expired: %s", mKey.c_str());
            }
        }
...
public:
    ...
    void setItem(const std::string& key, const sp<IBinder>& item) {
        ...
        if (status != android::OK) {
            ALOGE("Failed to linkToDeath binder for service %s. Error: %d", key.c_str(), status);
            return;
        }
        const std::lock_guard<std::mutex> lock(mCacheMutex);
        mCache[key] = item;
...
private:
    ...
    void updateCache(const std::string& service_name, const os::Service& service);
