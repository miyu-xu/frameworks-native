...
public:
    ...
    bool removeItem(const std::string& key, const sp<IBinder>& who) {
        std::string trace_str = "BinderCacheWithInvalidation::removeItem " + key;
        binder::ScopedTrace aidl_trace(ATRACE_TAG_AIDL, trace_str.c_str());
        std::lock_guard<std::mutex> lock(mCacheMutex);
        if (auto it = mCache.find(key); it != mCache.end()) {
    ...
    binder::Status setItem(const std::string& key, const sp<IBinder>& item) {
        ...
        if (item->localBinder() == nullptr) {
            ...
            if (status != android::OK) {
                std::string trace_str = "BinderCacheWithInvalidation::setItem Failed LinkToDeath" +
