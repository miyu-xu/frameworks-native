...
static VkResult getProducerUsageGPDIFP2(
    ...
    const InstanceDriverTable& instance_dispatch,
...
static VkResult getProducerUsage(const VkDevice& device,
    ...
    if (instance_dispatch.GetPhysicalDeviceImageFormatProperties2 ||
        ...
        VkResult result = getProducerUsageGPDIFP2(
            pdev, create_info, swapchain_image_usage,
            create_protected_swapchain, instance_dispatch, producer_usage);
        if (result == VK_SUCCESS) {
            return VK_SUCCESS;
        }
