...
static binder::Status update_path_context_to_pkg_dir_of_storage_areas(const std::string& path) {
    ...
    // wrapped in a unique_ptr to avoid calling `freecon` in every error case
    std::unique_ptr<char> oldsecontext_c;
    char* inner_ptr = oldsecontext_c.get();
    if (::lgetfilecon(path.c_str(), &inner_ptr) < 0) {
        return error("Unable to read secontext for: " + path);
    }
    std::string_view oldsecontext = inner_ptr;
...
binder::Status InstalldNativeService::createAppDataLocked(
    ...
    if (flags & FLAG_STORAGE_CE) {
        ...
        if (android::security::unlocked_storage_api() && !uuid) {
            // For internal storage only, create the package's directory of storage areas.
...
binder::Status InstalldNativeService::destroyAppData(const std::optional<std::string>& uuid,
    ...
    if (flags & FLAG_STORAGE_CE) {
        ...
        if (android::security::unlocked_storage_api() && !uuid) {
            // Delete the package's directory of storage areas.
            auto storage_area_path = create_data_storage_area_package_path(userId, pkgname);
            ...
            if (delete_dir_contents_and_dir(storage_area_fbe_keys_path, true)) {
                res = error("Failed to delete the FBE keys in " + storage_area_fbe_keys_path);
            }
        }
    }
    if (flags & FLAG_STORAGE_DE) {
