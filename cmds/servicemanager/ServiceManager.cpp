...
#define PERFETTO_TRACE_THIS_FUNCTION_WITH_ARG(function_name, arg)         \
    PERFETTO_TE_SCOPED(service_manager, PERFETTO_TE_SLICE(function_name), \
                       PERFETTO_TE_ARG_STRING(#arg, arg.c_str()));

#define PERFETTO_TRACE_THIS_FUNCTION(function_name) \
    PERFETTO_TE_SCOPED(service_manager, PERFETTO_TE_SLICE_BEGIN(function_name));
...
Status ServiceManager::getService(const std::string& name, sp<IBinder>* outBinder) {
    PERFETTO_TRACE_THIS_FUNCTION_WITH_ARG(__func__, name);
...
Status ServiceManager::listServices(int32_t dumpPriority, std::vector<std::string>* outList) {
    PERFETTO_TRACE_THIS_FUNCTION(__func__);
...
binder::Status ServiceManager::getDeclaredInstances(const std::string& interface, std::vector<std::string>* outReturn) {
    PERFETTO_TRACE_THIS_FUNCTION_WITH_ARG(__func__, interface);
...
Status ServiceManager::getUpdatableNames([[maybe_unused]] const std::string& apexName,
    ...
    PERFETTO_TRACE_THIS_FUNCTION_WITH_ARG(__func__, apexName);
