...
Status ServiceManager::getService(const std::string& name, os::Service* outService) {
    ...
    if (accessorName.has_value()) {
        sp<IBinder> out = tryGetService(*accessorName, true);
        LOG(WARNING) << "aaaaa A service is being accessed through an accessor: " << name << ":"
                     << *accessorName << " found: " << out.get();
        *outService = os::Service::make<os::Service::Tag::accessor>(out);
    } else {
        sp<IBinder> out = tryGetService(name, true);
        LOG(WARNING) << "aaaaa A service without an accessor: " << name << " found: " << out.get();
        *outService = os::Service::make<os::Service::Tag::binder>(out);
    }
