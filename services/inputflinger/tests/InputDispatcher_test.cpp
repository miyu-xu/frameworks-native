...
// Still send inject motion events to window which alredy be touched.
TEST_F(InputDispatcherTest, AlwaysDispatchInjectMotionEventWhenAlradyDownForWindow) {
    std::shared_ptr<FakeApplicationHandle> application1 = std::make_shared<FakeApplicationHandle>();
    sp<FakeWindowHandle> window1 =
    ...
    // Inject UP event, but policyFlags is not POLICY_FLAG_PASS_TO_USER like screen off.
